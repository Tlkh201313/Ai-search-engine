...
export function PersonaSelector({ value, onChange, align = 'left', direction = 'down' }: Props) {
  const [open, setOpen] = useState(false);
  const ref = useRef<HTMLDivElement>(null);
  const current = PERSONA_MAP[value];
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        aria-expanded={open}
        title="Choose a research persona"
      >
        ...
        >
          ...
          {PERSONAS.map((p) => {
            ...
            return (
              ...
              >
                <PIcon className={cn('mt-0.5 h-4 w-4 shrink-0', active ? 'text-accent' : 'text-faint')} />
